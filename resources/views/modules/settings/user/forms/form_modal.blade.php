...
<div class="modal-body">
    <form action="{{ url("setting/user/save",[$user?->id ?? 0]) }}" method="POST" enctype="multipart/form-data"
          id="form_validation">
            ...
            <div class="col-sm-12 col-md-12">
                <div class="d-flex flex-column">
                    <div class="combobox-container">
                        <select class="form-select select2-custom" name="app_group_user_id" id="app_group_user_id"
                                required>
                            <option value="">Pilih Grup</option>
                            @foreach($userGroups as $key => $value)
                                <option
                                    value="{{$value->id}}" {{($user?->app_group_user_id == $value->id ? "selected" : "")}}>{{$value->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <div class="row mb-3">
            <label for="no_telepon" class="col-sm-12 col-md-12 col-form-label">Nomor Telepon</label>
            <div class="col-sm-12 col-md-12">
                <input type="text" name="no_telepon" class="form-control" id="no_telepon" value="{{$user?->no_telepon}}" required>
            </div>
        </div>

        <div class="row mb-3 align-items-center">
            <label for="status" class="col-sm-12 col-md-12 col-form-label">Jenis Kelamin</label>
                ...
                <div class="d-flex flex-column">
                    <div class="radio-container">
                        @foreach($statuses as $key => $value)
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="status-{{$key}}"
                                       value="{{$key}}" {{ ($user?->status ?? 'active') == $key ? "checked" : ""}}>
